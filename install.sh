...
if [ -f "$ISPCONFIG_INSTALLER" ] && [ -d "/usr/local/ispconfig/interface/web" ]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        # The integration installer deploys the panel files, the Apache
        # injection, the sudoers grant, the template store AND the systemd
        # watchdog. It used to only do the first three, with the watchdog set
        # up here, so running it on its own left stale templates behind and the
        # watchdog reverted the freshly deployed files within the hour.
